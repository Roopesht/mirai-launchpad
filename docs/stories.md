...
| ID | User Story | Acceptance Criteria | Priority | Status |
|---|---|---|---|---|
| ST-105 | As a Maintainer, I want a local-only content editor screen, so I can edit JSON content files without leaving the browser or hand-editing files in an external editor. | A route (e.g. `/dev/content`) lists every editable file under `src/data/*.json`. | Must | **Done** *(`/dev/content`)* |
| ST-106 | As a Maintainer, I want to select a content file and see/edit its raw JSON in one place, so I don't have to hunt for it in the file tree. ~~Revised 2026-09-13: field-by-field, not raw JSON — see epic-13 doc.~~ | Selecting a file loads its current on-disk contents into a text editor; switching files preserves unsaved-change warnings. **Revised**: loads into a field-by-field form (`SchemaForm`, generated from the file's own zod schema), not a raw-text editor — the original "raw JSON" scope decision was deliberately overturned mid-build after the user reviewed it and asked for real inputs. Unsaved-change confirmation still applies when switching files. | Must | **Done** *(scope changed from the original story text — see epic-13 doc's "Status" section for why)* |
| ST-107 | As a Maintainer, I want my edits validated against that file's existing zod schema before saving, so I can't save something the app will fail to load. | Save re-uses the same schema each `src/lib/content/*.ts` loader already validates against; validation errors are shown inline and block the save. | Must | **Done** *(each loader now also exports its schema for this purpose, e.g. `personalSchema`, `experienceFileSchema`)* |
| ST-108 | As a Maintainer, I want a valid save to write back to the real file on disk, so the change takes effect without a manual copy-paste. | A small Vite dev-server middleware (registered via `configureServer`, not shipped in the build) accepts the edited JSON and writes it to `src/data/<file>.json`, pretty-printed. | Must | **Done** *(`vite-plugin-content-editor.ts`; also rejects path-traversal attempts in the requested filename)* |
| ST-109 | As a Maintainer, I want this editor completely absent from the production build and deployed site, so it never becomes a security surface or an accidental "CMS" contradicting requirements.md §12. | The route only renders when `import.meta.env.DEV` is true; the write-back middleware is only ever registered on the dev server, never present in `dist/`. | Must | **Done** *(exceeded the original AC — see epic-13 doc: a runtime `import.meta.env.DEV` guard alone still left an unreachable-but-present chunk in `dist/`, verified empirically; fixed with a virtual module whose content differs by build command, so the dev pages are never even discovered by Rollup during a production build)* |
| ST-110 | As a Maintainer, I want clear success/error feedback after saving, so I know whether my change actually took effect. | UI shows a distinct success state and a distinct error state (validation failure or write failure). | Should | **Done** *(`role="status"` for success, `role="alert"` for both validation and save errors)* |
| ST-111 *(added 2026-09-13)* | As a Maintainer, I want the `/customize` route repurposed as a dev-tools hub with a button per tool (JSON editor, blog editor from Epic 14, and room for more later), so I have one obvious place to find them. | `/customize` shows a card/button per tool; a tool with no route yet (or not epic-13/14-complete) shows a disabled "Coming soon" state instead of a dead link. A prominent, always-visible notice states these tools only work locally (`npm run dev`) and are unavailable on the deployed site — not a small-print footnote. Outside a dev build (`import.meta.env.DEV` false), every tool button is disabled regardless of whether its route exists. | Must | **Done** *(JSON Content Editor card flipped to enabled once ST-105-110 landed)* |
