...
- **Icon field convention** (`socials.json.icon`, `hobbies.json.icon`): a lucide-react icon *name* (PascalCase, e.g. `"Globe"`, `"BookOpen"`), resolved via a small explicit `ICON_MAP` in `src/lib/icons.tsx` — a curated map of statically-imported icons, deliberately not a lookup into the full lucide-react icon set (that ballooned the bundle ~600KB, and lucide's per-icon-chunk `dynamic` entry point split the build into 1000+ tiny chunk files — bad tradeoffs for a template shipping a handful of icons). **Adding a new icon name to content JSON requires a one-line addition to `ICON_MAP`** — the Maintainer Guide (Epic 10) must document this. An unmapped name falls back to a generic link icon rather than rendering nothing. Note: the installed lucide-react version ships no brand/logo icons (no `Github`, `Linkedin`, `Twitter`, etc. — dropped from the package) — `socials.json` uses generic icons instead (e.g. `"Code2"` for GitHub).
